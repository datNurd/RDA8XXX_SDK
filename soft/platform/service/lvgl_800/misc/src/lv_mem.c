...
#define COPY32 *d32 = *s_32; d32++; s_32++;
#define COPY8 *d8 = *s_8; d8++; s_8++;
...
{
    uint8_t * d8 = dst;
    const uint8_t * s_8 = src;

    lv_uintptr_t d_align = (lv_uintptr_t)d8 & ALIGN_MASK;
    lv_uintptr_t s_align = (lv_uintptr_t)s_8 & ALIGN_MASK;
    ...
    const uint32_t * s_32 = (uint32_t *)s_8;
    while(len > 32) {
        REPEAT8(COPY32)
        len -= 32;
    }
